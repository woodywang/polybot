...
WIN, BARS = 300, 5                    # 5-minute windows
...
def sigma_free(raw):
    ...
    print(f"  {'elapsed':<10}{'windows':>9}{'brownian':>11}{'actual':>9}"
          f"{'excess':>10}{'t':>8}")
    per = {}
        ...
        for t0, o, c, _sig, pts in windows(bars):
            for tau, s in pts:
                if s == o or c == o:
                    continue
                j = BARS - int(tau // 60)
                per.setdefault(j, {}).setdefault(t0, []).append(
                    1.0 if ((s > o) == (c > o)) else 0.0)
    for j in sorted(per):
        rho = math.sqrt(j / BARS)
        exp = 0.5 + math.asin(rho) / math.pi
        v = [statistics.mean(x) for x in per[j].values()]
        sd = statistics.stdev(v)
        d = statistics.mean(v) - exp
        print(f"  {f'{j}/{BARS}':<10}{len(v):>9,}{exp:>11.4f}"
              f"{statistics.mean(v):>9.4f}{d:>+10.4f}"
              f"{d/(sd/math.sqrt(len(v))):>+8.2f}")
